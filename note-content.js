...
// Curated alternatives for the bundled example notes; never replace edited source text.
function sampleModeNotes() {
  return {
    "os-why": [
      "가상 메모리는 각 프로그램에 자기만의 주소 공간을 보여주는 방식이다. 실제 메모리는 여러 프로그램이 나누어 쓰지만, 프로그램은 연속된 공간을 사용하는 것처럼 동작한다. 주소 변환은 가상 주소를 실제 메모리 위치에 연결하는 과정이다.",
      "가상 주소 공간과 물리 메모리를 분리하면 연속된 가상 페이지를 서로 떨어진 물리 프레임에 배치할 수 있다. 보호 권한 검사와 페이지별 매핑을 통해 격리와 공유를 함께 구현한다. 다만 주소 변환 자체에 비용이 들기 때문에 TLB 같은 캐시와 페이지 테이블 구조의 설계가 성능에 영향을 준다.",
      "가상 메모리의 목적: 주소 공간 분리 · 메모리 보호 · 효율적 적재.",
      "서로 다른 프로세스가 같은 가상 주소를 사용해도 되는 이유는?",
      "프로세스별 페이지 테이블이 같은 가상 주소를 서로 다른 물리 프레임에 연결할 수 있기 때문이다.",
    ],
    demand: [
      "요구 페이징은 지금 필요한 페이지만 메모리에 가져오는 방식이다. 프로그램 전체를 한꺼번에 올리지 않는다. 접근한 페이지가 없으면 운영체제가 디스크에서 가져온 뒤 실행을 이어간다.",
      "요구 페이징은 초기 적재 비용을 줄이지만 첫 접근 때 지연을 발생시킨다. 성능은 페이지 폴트 빈도와 처리 비용에 함께 좌우된다. 지역성이 높은 접근은 한 번 적재한 페이지를 여러 번 재사용하지만, 작업 집합이 사용 가능한 프레임보다 크면 교체가 반복될 수 있다.",
      "필요한 시점에 적재 → 페이지 미존재 시 폴트 → 적재 후 명령 재실행.",
      "요구 페이징에서 처음 접근한 페이지는 항상 메모리에 없는가?",
      "아니다. 이미 다른 경로로 적재되어 있거나 공유 중일 수 있다. 핵심은 필요할 때 적재한다는 정책이다.",
    ],
    process: [
      "페이지 폴트가 발생하면 먼저 접근이 허용된 주소인지 확인한다. 정상적인 접근이면 페이지를 놓을 프레임을 확보하고, 디스크에서 가져와 페이지 테이블을 갱신한다. 마지막으로 중단된 명령을 다시 실행한다.",
      "페이지 폴트는 단순히 디스크를 읽는 작업만이 아니다. 유효성 검사, 프레임 선택, 필요 시 더티 페이지 쓰기, 페이지 적재와 매핑 갱신을 포함한다. 처리 중 프로세스가 대기하면 스케줄러가 다른 프로세스를 실행할 수 있어 I/O 지연과 CPU 이용률을 함께 생각해야 한다.",
      "유효성 검사 → 프레임 확보 → 페이지 적재 → 테이블 갱신 → 중단된 명령 재실행.",
      "페이지 폴트 처리 후 다음 명령으로 바로 넘어가는가?",
      "아니다. 중단된 명령부터 다시 실행한다. 잘못된 주소 접근은 정상 페이지 적재와 별도로 처리한다.",
    ],
    "os-replace": [
      "빈 프레임이 없으면 기존 페이지 중 하나를 내보내야 한다. FIFO는 먼저 들어온 페이지를, LRU는 가장 오래 사용하지 않은 페이지를 고른다. OPT는 앞으로 가장 늦게 사용할 페이지를 고르는 이론적 비교 기준이다.",
      "교체 정책은 폴트 수와 관리 비용 사이의 절충이다. LRU는 과거 사용 이력을 미래의 근사로 사용하지만 정확한 순서 추적에는 비용이 든다. FIFO는 구현이 간단하지만 프레임 증가가 폴트 감소를 보장하지 않는다. OPT는 미래 참조를 알아야 하므로 실시간 구현보다 성능 하한 비교에 사용한다.",
      "FIFO: 적재 순서 / LRU: 최근 참조 이력 / OPT: 미래 참조 시점. FIFO의 벨라디 역설 주의.",
      "LRU와 FIFO가 다른 페이지를 고를 수 있는 이유는?",
      "오래전에 적재됐어도 최근에 자주 참조했을 수 있기 때문이다. 적재 시점과 마지막 참조 시점은 다르다.",
    ],
    "memory-tlb": [
      "TLB는 최근에 사용한 주소 변환 결과를 기억하는 작은 캐시다. 찾는 정보가 있으면 페이지 테이블을 다시 읽는 일을 줄인다. TLB에 정보가 없어도 페이지 자체는 메모리에 있을 수 있다.",
      "TLB 적중률은 주소 변환 비용을 좌우한다. 문맥 전환 시 주소 공간 식별자를 활용하면 서로 다른 프로세스의 변환 정보를 구분할 수 있다. 페이지 크기를 키우면 같은 엔트리 수로 더 큰 범위를 커버하지만 내부 단편화와 적재 단위도 함께 고려해야 한다.",
      "TLB 미스 ≠ 페이지 폴트. TLB는 변환 정보, 페이지 테이블은 페이지 매핑을 관리.",
      "TLB 미스 이후에도 디스크 I/O 없이 실행을 계속할 수 있는가?",
      "가능하다. 페이지 테이블에서 메모리에 있는 프레임을 찾으면 디스크에서 페이지를 가져올 필요가 없다.",
    ],
    "dp-steps": [
      "먼저 dp[i]가 어떤 답을 저장하는지 문장으로 정한다. 그다음 작은 문제의 답을 합쳐 현재 답을 만드는 점화식을 세운다. 시작할 값과 계산 순서를 정하면 표를 차례로 채울 수 있다.",
      "상태 설계는 미래 결정에 필요한 정보를 빠짐없이 포함하면서 중복을 줄이는 과정이다. 점화식의 각 전이가 모든 가능한 선택을 포함하는지, 부분 문제의 최적해로 전체 최적해를 구성할 수 있는지 검증한다. 의존 관계가 순환하면 단순한 표 채우기 순서를 바로 정할 수 없다.",
      "상태 의미 → 점화식 → 초기값 → 계산 순서 → 복잡도 검증.",
      "점화식이 맞아도 반복문 순서가 틀리면 왜 오답이 되는가?",
      "참조하는 이전 상태가 아직 계산되지 않았거나 같은 단계의 갱신된 값을 잘못 사용하기 때문이다.",
    ],
    "dp-cost": [
      "DP의 실행 시간은 대체로 상태의 수에 각 상태를 계산하는 비용을 곱해 구한다. 계단 문제는 n개 상태를 한 번씩 계산하므로 O(n)이다. 직전 두 값만 쓰면 배열 대신 두 변수로 기억할 수도 있다.",
      "공간 최적화는 상태 의존 관계가 제한될 때 가능하다. 그러나 최적값 계산에 충분한 정보와 실제 선택 경로 복원에 필요한 정보는 다르다. 순환 배열이나 역순 갱신을 사용할 때는 기존 값의 생존 구간을 분석해 덮어쓰기가 안전한지 증명해야 한다.",
      "시간 ≈ 상태 수 × 전이 비용. 직전 두 값만 필요하면 공간 O(1). 경로 복원 정보는 별도.",
      "DP 공간을 O(1)로 줄이면 항상 선택 경로도 복원할 수 있는가?",
      "아니다. 최적값만 남기면 어떤 선택을 했는지 사라질 수 있으므로 추가 저장이나 재계산이 필요하다.",
    ],
    "linear-condition": [
      "선형 변환은 벡터를 먼저 더한 뒤 변환해도, 각각 변환한 뒤 더해도 같은 결과를 낸다. 벡터를 몇 배로 늘리는 연산과도 순서를 바꿀 수 있다. 이 두 조건 때문에 영벡터는 영벡터로 간다.",
      "덧셈과 스칼라 곱 보존을 결합하면 T(au+bv)=aT(u)+bT(v)를 얻는다. 따라서 기저에서의 변환값만으로 모든 벡터의 변환을 결정할 수 있다. T(0)=0은 필요조건이지만 충분조건은 아니므로 원점을 보존한다는 사실만으로 선형성을 결론 내리면 안 된다.",
      "T(u+v)=T(u)+T(v), T(cu)=cT(u). T(0)=0은 필요조건이며 충분조건이 아님.",
      "T(x)=x²는 T(0)=0인데 선형 변환인가?",
      "아니다. 예를 들어 T(1+1)=4이지만 T(1)+T(1)=2라서 덧셈을 보존하지 않는다.",
    ],
    "linear-basis": [
      "행렬의 첫 열은 첫 번째 기저벡터가 이동한 결과이고, 둘째 열은 두 번째 기저벡터가 이동한 결과다. 모든 벡터는 기저벡터의 합으로 표현되므로 이 결과들을 같은 비율로 합하면 된다.",
      "선형성으로부터 A(x₁e₁+x₂e₂)=x₁Ae₁+x₂Ae₂가 성립한다. 행렬의 열은 선택한 입력 기저벡터의 상을 출력 기저로 표현한 좌표다. 따라서 같은 선형 변환도 기저를 바꾸면 다른 행렬로 표현될 수 있다.",
      "A의 i번째 열 = Aeᵢ. Ax는 열벡터들의 선형 결합.",
      "A의 열벡터들을 알면 왜 모든 Ax를 계산할 수 있는가?",
      "x를 기저벡터들의 선형 결합으로 쓰고 선형성을 적용할 수 있기 때문이다.",
    ],
    "linear-compose": [
      "ABx에서는 B가 먼저 x에 작용하고 A가 나중에 작용한다. 함수를 연달아 적용하는 것과 같다. 서로 다른 축으로 확대하는 일과 회전하는 일은 순서를 바꾸면 결과가 달라질 수 있다.",
      "행렬 곱은 변환 합성의 좌표 표현이다. B의 출력 공간과 A의 입력 공간 차원이 맞아야 AB를 정의할 수 있다. 합성은 결합법칙을 만족하지만 일반적으로 교환법칙은 만족하지 않으므로 차원 호환성과 적용 순서를 분리해 확인해야 한다.",
      "ABx: B → A 순서. 일반적으로 AB ≠ BA. 곱셈의 차원 조건 확인.",
      "A가 m×n, B가 n×p일 때 AB는 어떤 크기인가?",
      "m×p이다. 입력은 p차원, B의 출력은 n차원, A의 출력은 m차원이다.",
    ],
  };
}
